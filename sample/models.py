...
class FlightBookingDetails(models.Model):
    ...
    flight = models.ForeignKey('Flights', on_delete=models.CASCADE)  # Reference to the flight
    booking_date = models.DateTimeField(auto_now_add=True)  # Automatically sets the booking time
    seats_booked = models.PositiveIntegerField()  # Number of seats booked
    total_fare = models.DecimalField(max_digits=10, decimal_places=2)  # Total fare for the booking
...
class BusBookingDetails(models.Model):
    ...
    bus = models.ForeignKey(Buses, on_delete=models.CASCADE, related_name="bus_bookings")
    # seats_booked = models.IntegerField(blank=False)
    # total_fare = models.DecimalField(max_digits=10, decimal_places=2, blank=False)
    ...
    def __str__(self):
        return f"User: {self.user.email} | Bus: {self.bus.travels}"